...
vector<vector<ll> > dp(MAX, vector<ll>(MAX, -1L));

int recurse(int h, int w){
    if ( h < 0 || w < 0 ) return 0L;
    if( grid[h][w] == '#' ) return 0L;
    if( h == 0 && w == 0 ) return 1L;
    if( dp[h][w] != -1L ) return dp[h][w] % modd;
    return dp[h][w] = (recurse(h-1, w) % modd+ recurse(h, w-1) % modd) % modd;
}
