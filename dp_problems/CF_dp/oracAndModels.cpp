...
//dp problem - given is an array of sizes
//I can only include an element appearing ahead in the list if -
// 1. The index is a factor of the current index
// 2. The number there is greater than the current one.
// dp[index] = over(i=divisors of the index)max(dp[index/i])
// ans = max(dp);
    ...
    for( int i = 2; i <= n; i++  ){
        for( int j = 1; j <= sqrt(double(i)); j++){
            if(i%j == 0){
                ...
                if( arr[i/(i/j)] < arr[i] ) dp[i] = max(dp[i], dp[i/(i/j)]+1);
            }
        }
    }
    // print(dp, n+1);
