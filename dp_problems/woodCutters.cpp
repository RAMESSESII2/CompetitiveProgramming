...
void run_case(){
    ...
    loop(i, 2, n){
        ...
        if( height[i-2].ss < pos[i-1] ){
            dp[i][1] = max(dp[i][1], dp[i-1][2]);
        }
        if( height[i-1].ff > pos[i-2] ){
            dp[i][0] = 1 +max( dp[i-1][1], dp[i-1][0]);
        }
        if( height[i-1].ff > height[i-2].ss ){
    ...
    dp[n][2] =1+ max(dp[n-1][0], dp[n-1][1]);
    if( height[n-2].ss < pos[n-1] ){
        dp[n][2] = max(dp[n][2], 1+dp[n-1][2]);
    }
    cout << dp[n][2]<< endl;
