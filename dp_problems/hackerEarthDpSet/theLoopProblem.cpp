...
void run_case(){
    ...
    for( int bit = 30; bit >= 0; bit-- ){
        int firstOne = get(b, bit) - get(a-1, bit);
        int secondOne = get(d, bit) - get(c-1, bit);
        int firstZero = b-a+1-firstOne;
        int secondZero = d-c+1-secondOne;
        int val = binpow(2, bit, modd);
        int cur = ((firstZero*1L*secondOne)%modd + (firstOne*1L*secondZero)%modd)%modd;
        cur = (cur*1L*val)%modd;
        ans = ((ans%modd)+(cur%modd))%modd;
    }
    cout << ans << endl;
}
