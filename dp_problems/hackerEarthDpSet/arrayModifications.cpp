...
void run_case(){
    ...
    loop(i, 1, n){
        ...
        suff[n-i-1] = (suff[n-i] + arr[n-i-1])/2;
    }
    // print(pref, n);
    // print(suff, n);
    ll ans = 0L;
    ans = max(ans, arr[0]+ suff[1]);
    ans = max(ans, arr[n-1]+ pref[n-2]);
    loop(i, 1, n-1){
        ans = max(ans, suff[i+1]+pref[i-1]+ arr[i]);
        // cerr << suff[i+1]+pref[i-1]+ arr[i] << endl;
    }
    cout << ans << endl;
}
