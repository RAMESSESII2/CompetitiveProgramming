...
int steps(int pos){
    ...
    int ans = inf;
    if( pos+1 < n && cave[pos+1] != '*' )ans = min(ans, 1+steps(pos+1));
    if( pos+2 < n && cave[pos+2] != '*' )ans = min(ans, 1+steps(pos+2));
    int a = 0;
    a = lower_bound(all(primes), pos+1) - primes.begin() ;
    if( primes[a] == pos+1 ) a++;
    if(( a*r2*1L >= (pos+1L)*r1  && (pos+a) < n) && (cave[pos+a] != '*'))ans = min(ans, 1+steps(pos+a));
    return dp[pos] = ans;
}
