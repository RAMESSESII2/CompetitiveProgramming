...
void lis(){
    ...
    p.resize(n+1);
    temp.resize(n+1, 0);
    temp_id.resize(n+1, 0);
    loop(i, 0, n){
        int pos = lower_bound(temp.begin(), temp.begin()+l, arr[i]) - temp.begin();
        temp[pos] = arr[i];
        temp_id[pos] = i;
        p[i] = pos > 0 ? temp_id[pos-1] : -1;
        if( pos == l ){
            l++;
            l_end = i;
        }
        ...
        cerr << "temp[pos]:  " << temp[pos]  << "temp_id[pos]: " << temp_id[pos] << " p[i]: "<< p[i]<< endl;
        cerr<< "l: " << l << "  l_end: " << l_end << endl; 
        cerr << endl;
    }
    loop(i, 0, sz(temp)){
        cerr << temp[i] << " " << " " << temp_id[i] << " " << p[i] << endl;
    }
    cout  << l << endl;
    cout << "-\n";
    printt(l_end);
