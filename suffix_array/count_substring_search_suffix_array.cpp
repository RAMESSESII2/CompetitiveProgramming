...
int binary_Search(int count, string s,const vector<int> &p,string ssearch, ll low, ll high){
    /* ll low = 0; */
    /* ll high = sz(s)-1; */
    /* if(low > high){ */
    /*     return 0; */
    ...
    while(low <= high){
        ...
        if (ssearch == c){
            count++;
            if(low<high){
            count += binary_Search(0, s, p, ssearch, low , m-1)+binary_Search(0, s, p, ssearch, m+1 , high);
            }
            return count;
        }
        else{
...
void run_case(){
    ...
    while(z--){
        ...
        ll low = 0;
        ll high = sz(s)-1;
        int res = binary_Search(0, s, p, sstr, low , high);
        if (res == 0){
            cout << res << endl;
        }
        else{
