...
void run_case(){
    ...
    for( int i = 0; i < n; i++){
        loop(j, 0, m){
            if( j+1 != m ){
                edges.push_back({grid[i][j]^grid[i][j+1],{encode(i, j), encode(i, j+1)}} );
            }
            if( i+1 != n ){
                edges.push_back({grid[i][j]^grid[i+1][j],{encode(i, j), encode(i+1, j)}} );
            }
        }
    }
    sort( all( edges ) );
    ...
    ll res = 0;
    for( int i = 0; i < edges.size(); i++){
        int wt = edges[i].ff, a = edges[i].ss.ff, b= edges[i].ss.ss;
