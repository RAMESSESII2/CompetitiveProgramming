...
int findPark(int pos){
    return parent[pos] = (parent[pos] == pos ?  pos : (findPark(parent[pos])));
}

void nextPark(int pos, int next){
    int x = findPark(pos);
    int y = findPark(next);
    if( x == y ) return;
    // cerr << "pos: " << pos << " find: " << x <<endl ;
...
void run_case(){
    ...
    loop(i, 1, n+1){
        int pos;
        cin >> pos;
        int ans = findPark(pos);
        cout << ans << endl;
        if( ans == n ){
            nextPark(ans, 1);
        }
        else nextPark(ans, ans+1);
    }
    // loop(i, 0 ,n){
    //     cerr << parent[i+1] << " " << rankk[i+1] << endl;
    // }
}
