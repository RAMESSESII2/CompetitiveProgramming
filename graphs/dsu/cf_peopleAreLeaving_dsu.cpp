...
int findSet(int v){
    if( v == n+1 ) return -1;
    return parent[v] = ((parent[v]==v)?v:((v == n) ? -1 :findSet(parent[v])));
}
 
void union_(int a){
    int x = findSet(a+1);
    if( rankk[x] == rankk[a] )rankk[x]++;
    maxm[a] = x;
    parent[a] = x;
}
...
void run_case(){
    ...
    // cerr << "parent, max " << parent[n] << " " << maxm[n] << " " << findSet(n) <<  endl; 
    // cerr << "parent, max " << parent[n-1] << " " << maxm[n-1] <<  " " << findSet(n-1) << endl; 
}
