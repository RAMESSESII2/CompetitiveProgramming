...
ll bfs( pll init, pll fin ){
    ...
    while( !deq.empty() ){
        ...
        for( int i = 0; i<8; i++ ){
            ll r = x.ff + dirx[i];
            ll c = x.ss + diry[i];
            pll cur = make_pair(r, c);
            if( mp[cur] == 1  && visited[cur]==0 ){
                // cerr << r << " " << c << endl;
                deq.push(cur);
                visited[cur] = visited[x] + 1;
                // cerr << cur.ff << " " << cur.ss << " till now: " << visited[x]+1 << endl;
            }
        }
    }
    // cerr << visited[make_pair(10, 243)] << endl;
    if( visited.find( fin ) != visited.end() ){
        return visited[fin];
    }
    else return -1;
}
...
void run_case(){
    ...
    pll init = {inx, iny};
    pll fin = {fix, fiy};
    // cerr << init.ff << " " << init.ss << "    " << fin.ff << " " << fin.ss << endl;
    cin >> n;
    loop(i, 0, n){
        ll row, sc, ec;
        cin >> row >> sc >> ec;
        loop(j, sc , ec+1){
            mp[make_pair(row, j)] = 1;
        }
    }
