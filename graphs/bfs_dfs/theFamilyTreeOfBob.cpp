...
void dfs(int v, vi p){
    if( visited[v] ) return;
    visited[v] = 1;
    ancestors[v].assign(all(p));
    vi vec;
    vec.pb(v);
    vec.insert(vec.begin()+1, all(p));
    for( auto x: adj[v]){
...
void run_case(){
    ...
    loop(i, 0, m){
        cin >> u >> k;
        // cerr << sz(ancestors[u]) << endl;
        // cerr << u << " " << k << endl;
        if( k == 0 ){ 
            cout << u << endl;
        }
        if( k+1 > sz(ancestors[u]))cout << -1 << endl;
        else{
            cout << ancestors[u][k-1] << endl;
        }
    }
}
