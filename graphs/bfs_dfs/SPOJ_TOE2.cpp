...
void run_case(){
    while( true ){
        ...
        // print2( dp, 0, 3, 0, 3 );
        won();
        pair<bool, bool> winss = {(wins.ff > 0)?1:0, (wins.ss > 0)?1:0};
        cerr << "countO: " << countO << " countX: " << countX << endl;
        cerr << "X: " << wins.ff << " O : " << wins.ss << endl;
        bool flag = 1;
        if( winss == make_pair(true, false) && countX - countO == 1 ){
            cout << "valid\n";
            flag = 0;
        }
        if( winss ==make_pair(false, true) && countO == countX ){
            cout << "valid\n";
            flag = 0;
        }
        if( winss ==make_pair(false, false)  && countX == 5 && countO == 4){
            cout << "valid\n";
            flag = 0;
        }
