...
void run_case(){
    ...
    loop(i, 0, n){
        ...
        int heroTakes = ceil(a[i].ss/A*1.0);
        int monsterTakes = ceil(B/a[i].ff*1.0);
        if( monsterTakes < heroTakes ){
            cout << "NO\n";
            return;
        }
        float monsterHealth = 1.0*a[i].ss-heroTakes*1.0*A;
        if( monsterHealth > 0 ){
            cout << "NO\n";
            return;
        }
        B -= heroTakes*a[i].ff*1.0;
    }
    cout << "YES\n";
}
