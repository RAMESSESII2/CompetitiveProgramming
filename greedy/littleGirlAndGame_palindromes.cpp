...
void run_case(){
    ...
    int findEvenOcc = 0;
    for( auto x : s) mp[x]++;
    auto palindrome = [&]()->bool{
        findOddOcc = 0;
        findEvenOcc = 0;
        for( auto x: mp ){
            if( (x.ss&1) && x.ss > 0) findOddOcc++;
            if ( x.ss > 0 && !(x.ss&1) ) findEvenOcc++;
        }
        if( findOddOcc > 1 ) return false;
        return true;

    };
    for( int i =0 ; ; i++ ){
        // cerr << findEvenOcc << " " << findOddOcc <<endl;
        // if( i == 10 ) break;
        if( palindrome() ){
            cout << (!(i&1) ? "First\n":  "Second\n");
            break;
