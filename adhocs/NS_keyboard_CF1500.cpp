...
void run_case(){
    ...
    // loop(i, 0, sz(sarr)){
    //     cerr << sarr[i].ff << " " << sarr[i].ss << endl;
    // }
    cerr << endl;
    for( int i = 1; i<sz(sarr); i++ ){
        for( int row = 0; row < n; row++ ){
            for(int col = 0; col < m ; col++ ){
                int left = pow(abs(row - sarr[i].ff),2); 
                int right =pow(abs(col-sarr[i].ss ),2);
                int cur = (left + right );
                euclid[dp[row][col]] = min( euclid[dp[row][col]], cur);
            }
        }
    }
    int q;
    ...
    loop(i, 0, q){
        char ch;
        cin >> ch;
        if( ch>=97 && ch <= 122 && euclid.find(ch) != euclid.end()  ){
            continue;
        }
        else if( ch+32>=97 && ch+32 <= 122  ){
            if( euclid.find(ch+32) != euclid.end() && euclid.find('S') != euclid.end()){
                int eucdist = euclid[ch+32];
                if( eucdist > x*x ) count++;
                cerr << ch << " " << eucdist << " count : " << count << endl;
            }
            else{ 
                cout<< -1 << endl;
                return;
