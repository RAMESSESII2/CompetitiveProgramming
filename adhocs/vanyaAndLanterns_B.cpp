...
void run_case(){
    ...
    ll maxmDist = -8;
    loop(i, 1, n){
        maxmDist = max( maxmDist, arr[i] - arr[i-1] );
    }
    ll another = -7;
    if( arr[0] != 0  ) another = max(another, arr[0]);
    if( arr[n-1] != l  ) another = max(another, l-arr[n-1]);
    if( another*1.0 > (maxmDist*1.0)/2.0 ){
        cout << fixed <<  setprecision(9) << another*1.0<< endl;

    }
    else
        cout << fixed <<  setprecision(9) << (maxmDist*1.0)/2.0 << endl;
}
