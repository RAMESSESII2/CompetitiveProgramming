...
void run_case(){
    ...
    ans.clear();
    ans.resize(n);
    double firstpart = 0, secpart = 0;
    loop(i, 0, n){ 
        cin >> a >> b;
        ans[i].ss = i+1;
        firstpart = a*t1-(double(a*t1*k)/100);
        firstpart += t2*b;
        secpart = t1*b-(double(t1*b*k/100.0000));
        secpart += t2*a*1.0000;
        ans[i].ff = -1*max(secpart, firstpart);
    }
    sort(all(ans));
    loop(i, 0, n){ 
        cout << ans[i].ss << " ";
        cout <<fixed << setprecision(2) <<  -1*ans[i].ff << endl;
    }
}
