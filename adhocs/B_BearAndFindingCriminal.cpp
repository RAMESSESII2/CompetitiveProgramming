...
void run_case(){
    ...
    //    ^
    //
    int back = currPos - 1, front = currPos + 1;
    int count = 0;
    while( true ){
        if( back >= 0 and front < n ){
            if( arr[back] + arr[front] == 2 ){
                count += 2;
            }
            back--;
            front++;
        ...
        else if ( back >= 0 && arr[back] ==1 ){
                count++;
                back--;
                continue;
        }
        else if ( front < n && arr[front] ==1 ){
                count++;
                front++;
                continue;
        }
        if( front >= n && back < 0 ) break;
        back--;
        front++;
    }
    if( arr[currPos] ) count++;
    cout << count << endl;
}
