...
void run_case(){
    cin >> n;
    rValues.resize(n);
    for( auto &x : rValues ) cin >> x;
    sort(all(rValues));
    cin >> m;
    p1Values.resize(m);
    for( auto &x : p1Values ) cin >> x;
    sort(all(p1Values));
    cin >> k;
    p2Values.resize(k);
    for( auto &x : p2Values ) cin >> x;
    sort(all(p2Values));
    cin >> A >> B;
    auto calc = [=](int r1, int p1, int p2)->double{ 
        double rat = sqrt( (A*p2*1.0)/(B*p1*1.0) + 1 );
        double req = (r1*1.0)/rat;
        cerr << req << endl;
        return req;
    };
    double ans = -inf;
    ans = calc(rValues[sz(rValues)-1], p1Values[sz(p1Values)-1], p2Values[0]);
