...
void run_case(){
    ...
    loop(i, 0, t){
        ...
        cin >> x >> y;
        // cout << x << " " << y << endl;
        auto nearest = lower_bound(all(waste), make_pair(x,y));
        if( nearest->ff == x && nearest->ss == y ){
            // cerr << "WASTE :" <<x << " " << y << endl;
            cout << w << endl;
            continue;
        }
        nearest--;
        while( nearest->ff == x && nearest->ss > y){
            nearest--;
        }

        // cerr << nearest->ff << " " << nearest->ss << endl;
        int cur = ((x-1)*m + y-1 -(nearest-waste.begin()))%3;
        cout << ((cur == 0)? ca :((cur == 1) ? ki: gr)) << endl;
        // cerr << nearest-waste.begin() << " cur" << cur << endl;
    }
}
