...
void run_case(){
    cin >> n >> k;
    vector< pll > arr(n);
    loop(i, 0, n) cin >> arr[i].ff >> arr[i].ss;
    sort( all(arr), [](pll a, pll b)->bool{
            ll t = min( 2*a.ff,a.ss ) - min( a.ff, a.ss );
            ll u = min( 2*b.ff,b.ss ) - min( b.ff, b.ss );
            return t > u;
            });
    // loop(i, 0, n){
