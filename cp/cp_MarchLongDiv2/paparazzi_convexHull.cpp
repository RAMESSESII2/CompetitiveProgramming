...
void run_case(){
    ...
    ll ans = 1;
    vector<pair<ll, ll> > poly{pt[0], pt[1]};
    for( ll i = 2; i < n; i++ ){
        auto crossProduct = [&](){
            ll siz = int(poly.size());
            auto [x, y] = poly[siz-2];
            auto [x1, y1] = poly[siz-1];
            auto [x2, y2] = pt[i];
            return (x1- x) * (y2-y) - (x2-x) * (y1 - y);
        };
        while( poly.size() > 1 and crossProduct() >= 0)poly.pop_back();
        ans = max(ans, pt[i].ff - poly.back().ff);
        poly.pb(pt[i]);
    }
    cout << ans << endl;
}
