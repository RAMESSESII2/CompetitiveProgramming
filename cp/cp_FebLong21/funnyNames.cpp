...
void run_case(){
    ...
    loop(i, 0, n){
        ...
        cin >> x;
        funny[i] = x;
        mp1[x.substr(1)].insert(x[0]);
        mp2[x[0]].insert(x.substr(1));
    }
    int count = 0;
    for( auto x = mp1.begin(); x != mp1.end(); x++ ){
        ...
        while( yt != mp1.end()){
            set<int> meerged;
            set_union(((*yt).ss).begin(), ((*yt).ss).end(), ((*x).ss).begin(), ((*x).ss).end(), inserter(meerged, meerged.begin()));
                // if(((*yt).ss).find(a) == ((*yt).ss).end()) count+=2;
            int cur = sz(meerged);
            count += (cur-sz(((*yt).ss)))*(cur-sz(((*x).ss)));
