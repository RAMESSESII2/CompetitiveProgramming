...
void run_case(){
    ...
    cin >> n;
    // cerr << t << m << endl;
    auto convert = []( string e, string em )->pii{
        int hr, min;
        if(em == "AM"){
            if( e.substr(0, 2) == "12" ){
                hr = 0;
            }
            else hr = stoi(e.substr(0, 2));
            min = stoi(e.substr(3, 2));
        }
        else {
            if( e.substr(0, 2) == "12" ){
                hr = 12;
            }
            else hr = (12+stoi(e.substr(0, 2)));
            min = stoi(e.substr(3, 2));
        }
    ...
    loop(i, 0, n){
        ...
        tie(lefthr, leftmin) = convert(l, lm);
        tie(righthr, rightmin) = convert(r, rm);
        // cerr << lefthr << " " << leftmin << "  ";
        // cerr << righthr << " " << rightmin << endl;
        if( lefthr > hr || righthr < hr ){
            cout << 0;
            continue;
        }
        if( lefthr == hr ){
