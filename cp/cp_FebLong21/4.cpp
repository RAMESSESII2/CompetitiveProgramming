...
void run_case(){
    cin >> n;
    //vector of tuples<wt int , index int, jumpDist int>
    vector< pair<pii , int > > wtIndDist(n+1);
    loop(i, 1, n+1){
        cin >>wtIndDist[i].ff.ff;
        wtIndDist[i].ff.ss = i;
    }
    loop(i, 1, n+1){
        cin >> wtIndDist[i].ss;
    ...
    int count = 0;
    for( int i = 2; i<n+1; i++ ){
        if( wtIndDist[i].ff.ss <= wtIndDist[i-1].ff.ss ){
            int newpos = wtIndDist[i].ff.ss;
            while( newpos <= wtIndDist[i-1].ff.ss ){
                newpos += wtIndDist[i].ss;
                count++;
            }
            wtIndDist[i].ff.ss = newpos;
        }
    }
    cout << count << endl;
