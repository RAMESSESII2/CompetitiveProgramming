...
void run_case(){
    cin >> n >> k;
    vi blogger(n);
    for( auto &x : blogger ) cin >> x;
    sort(allr(blogger));
    /* print( blogger, n ); */
    if( k >= n || blogger[k] != blogger[k-1] ){
        cout << 1 << endl;
        return;
    }
    int start = k;
    int end = k+1;
    while( start > 0 && blogger[start-1] == blogger[start] )start--;
    while( end < n && blogger[end-1] == blogger[end] )end++;
    /* cout << start << " " << end << endl; */
    /* cout << ( end-start+1) <<" " <<( k-start ) <<endl; */
    cout << choose(end-start, k-start) << endl;
}
