...
void run_case(){
    ...
    //dp[i][j] = dp[i-1][k] where k > j;
    //dp[i][j] = dp[i-1][j] where if = ;
    loop(i, 1, n+1){
        dp[1][i] = 1;
    }
    loop(i, 2, n+1){
        if( arr[i-2] == '<'){
            loop(j, 1, n+1) dp[i][j] = (dp[i][j-1] + dp[i-1][j-1])%modd;
        }
        else if( arr[i-2] == '>'){
            loopr(j, n, 0) dp[i][j] = (dp[i][j+1] + dp[i-1][j+1])%modd;
        }
        else{
            loop(j, 1, n+1) dp[i][j] = dp[i-1][j];
        }
    }
    ll ans = 0L;
    loop(j, 1, n+1){
        ans = ((ans%modd) + (dp[n][j] %modd)) %modd;
    }
    cout << ans << endl;
}
