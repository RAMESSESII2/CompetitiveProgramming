...
string s;
int dp[1008];
int recurse(int pos, char prev){
    if( pos > sz(s)-1 ) return 0;
    if( s[pos] == 'J' ){
        if( prev == 'C' ) dp[pos] = x+recurse(pos+1, 'J');
        else dp[pos] = recurse(pos+1, 'J');
    }
    if( s[pos] == 'C' ){
        if( prev == 'J' ) dp[pos] = y+recurse(pos+1, 'C');
        else dp[pos] = recurse(pos+1, 'C');
    }
    if( s[pos] == '?'){
        dp[pos] =  min(recurse(pos+1, 'J'), recurse(pos+1, 'C'));
    }
    return dp[pos];
}
