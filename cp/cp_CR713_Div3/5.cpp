...
auto maxx(A a, B b) -> decltype(a > b ? a : b)
{
    return (a > b) ? a : b;
}
...
void run_case(){
    ...
    ll cur = 0L;
    loop(i, 0, n){
        ans = min(ans, cur+ maxx(0L, c-savings+salary[i]-1L)/salary[i]);
        numberOfDays = maxx(0L, b[i]-savings + salary[i]-1)/salary[i];
        cur += numberOfDays+1;
        savings += (salary[i]*numberOfDays) - b[i];
    }
    cout << ans << endl;
}
