...
void buildTree(vi a, int d){
    if( a.empty() == true) return;
    int maxele = *max_element(all(a));
    int maxeleIndex = max_element(all(a)) - a.begin();
    // cerr << " " << maxeleIndex  << " "<< endl;
    tree[maxele].ff = maxeleIndex;
    tree[maxele].ss = d;
    vi newleft(a.begin(), a.begin() + maxeleIndex);
    vi newright(a.begin() + maxeleIndex + 1, a.end());
    buildTree(newleft, d+1);
    buildTree(newright, d+1);
    return;
}
...
void run_case(){
    ...
    buildTree(arr, 0);
    // sort( all(tree), [](node *a, node *b)->bool{
