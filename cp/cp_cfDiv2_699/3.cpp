...
void run_case(){
    ...
    vvi needs(n+1), wants(n+1);
    for( int i = 0; i < n; i++ ){
        wants[arrB[i]].push_back(i);
        if( arrA[i] != arrB[i] ) needs[arrB[i]].pb(i);
    }
    if( wants[colors.back()].empty() ){
        cout << "NO" << endl;
        return;
    }
    vi where(m, -1);
    for( int i = m-1; i>=0; i-- ){
        if( !needs[colors[i]].empty()){
            where[i] = needs[colors[i]].back();
            needs[colors[i]].pop_back();
        }
        else if ( !wants[colors[i]].empty() ){
            where[i] = wants[colors[i]].back();
        }
        else{
            where[i] = where.back();
        }
