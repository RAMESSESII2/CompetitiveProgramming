...
void run_case(){
    ...
    bool l = false, u = false, num = false, an = false;
    auto special = [] (string a)->bool{
        if( a == "@" or a =="#" or a=="%" or a=="&" or a=="?"){
            return true;
        }
        return false;
    };
    auto upper = [](char a)->bool{
        if ((int(a) >= 65 && int(a) <= 90) ) return true;
        return false;
    };
    auto lower = [] (char input_char ) -> bool{
        if ((input_char >= 97 && input_char <= 122) ) return true;
        return false;
    };
    auto numm = [] (string a)->bool{
        for( int i =0; i<=9; i++ )
            if( (a) == to_string(i) ) return true;
        return false;
    };
    for( i = 0; i <s.length(); i++  ){
        if( special(s.substr(i, 1)) && i!=0 && i!=s.length() -1 ){
            an = true;
        }
        if( upper(s[i]) && i !=0 && i!= s.length()-1  ){
            u = true;
        }
        if( lower(s[i]) ){
            l = true;
        }
        if( numm(s.substr(i, 1)) && i != 0 && i!= s.length()-1 ){
            num = true;
        }
    }
    /* cout << an << " " << u << " " << l << " " << num << endl; */
    if( an && u && l && num ){
        cout << "YES" << endl;
        return;
     }
